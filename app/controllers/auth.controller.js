...
async function sendVerificationMail(fullname, email, resetToken, res) {
  ...
  try {
    ...
    const emailTemplateDTO = new EmailTemplateDTO(email, subject, template, data);
...
exports.verifyUser = async (req, res) => {
  ...
  try {
    ...
    if (req.body.email != null && req.body.email != undefined) {
      ...
      let isNewUser = false;
      if (tempuser != null && tempuser != undefined && (user == null || user == undefined)) {
        isNewUser = true;
      }
...
exports.verifiyOTP = async (req, res) => {
  ...
  try {
    ...
    if (req.body.email != null && req.body.email != undefined &&
      ...
      const tempUser = await TempUser.findOne({ where: { email: email, otp: code } });
...
exports.userGoogleSignIn = async (req, res) => {
  ...
  try {
    ...
    if (req.body.email != null && req.body.token != null && req.body.email != undefined
      ...
      if (user) {
        ...
        const passwordIsValid = bcrypt.compareSync(constants.DUMMY_PW, user.password);
    ...
    return res.status(httpResponseCode.HTTP_RESPONSE_INTERNAL_SERVER_ERROR).send({
      code: httpResponseCode.HTTP_RESPONSE_INTERNAL_SERVER_ERROR,
      message: 'Failed to signup. Please try again later', error
    });
  }
...
exports.userSignOut = async (req, res) => {
  ...
  try {
    ...
    return res.status(httpResponseCode.HTTP_RESPONSE_INTERNAL_SERVER_ERROR).send({
      code: httpResponseCode.HTTP_RESPONSE_INTERNAL_SERVER_ERROR,
      message: 'Failed to sign out. Please try again later', error
    });
  }
}
